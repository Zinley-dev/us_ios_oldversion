...
open class CWRateKitViewController: UIViewController {
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        super.touchesBegan(touches, with: event)
         ...
         if let touch = touches.first {
             let currentPoint = touch.location(in: self.rateView)
             
             if rateView.bounds.contains(currentPoint) {
    ...
    private func submit() {
        ...
        UIView.transition(with: self.rateView, duration: 0.2, options: .transitionCrossDissolve, animations: {
