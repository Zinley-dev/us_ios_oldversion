//
//  PhoneVerificationChangeVC.swift
//  Dual
//
...
class PhoneVerificationChangeVC: UIViewController, UITextFieldDelegate {
    ...
    func setupView() {
        ...
        vView.verifyBtn.addTarget(self, action: #selector(PhoneVerificationChangeVC.verifyBtnPressed), for: .touchUpInside)
        vView.resendCodeBtn.addTarget(self, action: #selector(PhoneVerificationChangeVC.resendCodeBtnPressed), for: .touchUpInside)
        vView.openKeyBoardBtn.addTarget(self, action: #selector(PhoneVerificationChangeVC.openKeyBoardBtnPressed), for: .touchUpInside)
                ...
                if let dict = json as? [String: AnyObject] {
                    ...
                    if let valid = dict["valid"] as? Bool {
                        ...
                        if valid == true {
                            ...
                            self.processSaveNewPhone(phone: phone, code: countryCode, userCode: self.UserCode!)
    ...
    func processSaveNewPhone(phone: String, code: String, userCode: String) {
        
        let data = ["phone": code + phone, "phone_verified": true] as [String : Any]
        ...
        DataService.instance.mainFireStoreRef.collection("Users").document(userCode).updateData(data) { (err) in
            ...
            SwiftLoader.hide()
            ActivityLogService.instance.UpdateAccountActivityLog(mode: "Update", info: "Phone")

            let alertController = UIAlertController(
                title: "Your new phone number has been saved!",
                message: nil,
